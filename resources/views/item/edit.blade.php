...
<div class="col-md-10 card ">
    ...
    <form action="{{ url('/items/edit')}}" method="POST"  class="card-body">
        {{ csrf_field() }}


        <input type="hidden" name="id" value="{{$item_edit->id}}">
        <input type="hidden" name="updateuser" value="{{ Auth::user()->id }}">
        ...
        <div class="form-group">
            <label for="item_edit_datail">詳細</label>
                <textarea id="item_edit_datail" name="detail" rows="5" class="form-control">{{old('detail', $item_edit->detail) }}</textarea>
                @error('detail')
                    <p>{{ $message }}</p>
                @enderror
        </div>
